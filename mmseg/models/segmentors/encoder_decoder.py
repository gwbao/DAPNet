...
from ..backbones.domain_invariant_enhancement import DomainInvariantEnhancer
...
class EncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.automatic_debug =  False #True1119
        self.debug = False
        self.debug_output = {}
    ...
    def extract_feat_with_enhancement(self, img, domain_labels, training=True):
        ...
        if self.domain_enhancer is None:
            ...
            self.domain_enhancer_cfg['num_channels'] = num_channels
            self.domain_enhancer = DomainInvariantEnhancer(
                **self.domain_enhancer_cfg
            ).to(feat.device)

        if isinstance(x, (list, tuple)):
            enhanced_x = list(x)
            domain_logits, enhanced_feat = self.domain_enhancer(
                x[-1], domain_labels, training=training
            )
            enhanced_x[-1] = enhanced_feat
            return enhanced_x, domain_logits
        else:
            domain_logits, enhanced_x = self.domain_enhancer(
                x, domain_labels, training=training
            )
            return enhanced_x, domain_logits
    ...
    def forward_train(self,
        ...
        if self.use_domain_enhancer and domain_labels is not None:
            x, domain_logits = self.extract_feat_with_enhancement(
                img, domain_labels, training=True
            )
        ...
        loss_decode = self._decode_head_forward_train(
            x, img_metas, gt_semantic_seg, seg_weight, return_logits
        )
