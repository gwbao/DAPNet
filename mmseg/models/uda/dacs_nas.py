...
from mmseg.models.utils.Controller import Controller
...
class DACS_NAS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        if self.use_memory:
            ...
            self.memory_manager = Controller(
    ...
    def forward_train(self,
        ...
        Args:
            ...
            target_img_metas (list[dict]): Target image info dict.
            valid_pseudo_mask: Valid pseudo label mask.
        ...
        if self.use_memory and self.memory_manager is not None:
            with torch.no_grad():
                gt_seg_squeezed = gt_semantic_seg.squeeze(1)
                self.memory_manager(
                    images=img.clone(),
                    targets=gt_seg_squeezed.clone(),
                    method='w',
                    domain='s'
                )
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            self.choices = self.model.backbone.mrfunet.burnin(
                factor_prod, self.long_burnin, choices=None
            )

        # Select kernel for search
        ...
        source_domain_labels = torch.zeros(batch_size, dtype=torch.long, device=dev)

        # Train on source images with max5 architecture
        choices_one_hot_max = self.model.backbone.mrfunet.get_choices_one_hot(
            mode=f"max{kernel_size}"
        ).to(device=torch.cuda.current_device())

        clean_losses_max = self.get_model().forward_train(
            img,
            img_metas,
            gt_semantic_seg,
            choices_one_hot=choices_one_hot_max,
            return_feat=True,
        ...
        domain_logits_source = self.get_domain_losses()
        if domain_logits_source is not None:
            domain_loss_source = F.cross_entropy(
                domain_logits_source,
                source_domain_labels
            )
            log_vars['loss_domain_source_max'] = domain_loss_source.item()
            clean_losses_max['loss_domain'] = domain_loss_source
        ...
        # Train on source images with sampled architecture
        ...
        domain_logits_source2 = self.get_domain_losses()
        if domain_logits_source2 is not None:
            domain_loss_source2 = F.cross_entropy(
                domain_logits_source2,
                source_domain_labels
            )
            log_vars['loss_domain_source_sample'] = domain_loss_source2.item()
            clean_losses['loss_domain'] = domain_loss_source2
        ...
        if not self.source_only:
            ...
            if self.use_memory and self.memory_manager is not None:
                with torch.no_grad():
                    mem_sample = self.memory_manager(
                        method='r',
                        device=dev,
                        domain='s'
                    )
                    mem_images = mem_sample['images']
                    mem_targets = mem_sample['targets']

                    if mem_images is not None and len(mem_images) > 0:
                        source_img_for_paste = torch.stack(mem_images).to(dev)
                        source_label_for_paste = torch.stack(mem_targets).to(dev)

                        if source_img_for_paste.size(0) < batch_size:
                            n_repeats = (batch_size + source_img_for_paste.size(0) - 1) // source_img_for_paste.size(0)
                            source_img_for_paste = source_img_for_paste.repeat(n_repeats, 1, 1, 1)[:batch_size]
                            source_label_for_paste = source_label_for_paste.repeat(n_repeats, 1, 1)[:batch_size]
                    else:
                        source_img_for_paste = img
                        source_label_for_paste = gt_semantic_seg.squeeze(1)
            else:
                source_img_for_paste = img
                source_label_for_paste = gt_semantic_seg.squeeze(1)
            ...
            for i in range(batch_size):
                ...
                mixed_img[i], mixed_lbl[i] = strong_transform(
                    strong_parameters,
                    data=torch.stack((source_img_for_paste[i], target_img[i])),
                    target=torch.stack((source_label_for_paste[i], pseudo_label[i]))
                )
            ...
            mixed_domain_labels = torch.randint(
                0, 2, (batch_size,), dtype=torch.long, device=dev
            )

            # Sample architecture for mixed domain
            factor_prod = self.model.backbone.mrfunet.create_factor_prod()
            self.choices = self.model.backbone.mrfunet.burnin(
                factor_prod, self.short_burnin, self.choices
            )
            choices_one_hot = self.model.backbone.mrfunet.sample(
                factor_prod, self.tau, self.choices
            )
            ...
            mix_losses = self.get_model().forward_train(
                ...
                return_feat=False,
                domain_labels=mixed_domain_labels
            )

            seg_debug['Mix'] = self.get_model().debug_output

            domain_logits_mixed = self.get_domain_losses()
            if domain_logits_mixed is not None:
                domain_loss_mixed = F.cross_entropy(
                    domain_logits_mixed,
                    mixed_domain_labels
                )
                log_vars['loss_domain_mixed'] = domain_loss_mixed.item()
                mix_losses['loss_domain'] = domain_loss_mixed
        ...
        if self.local_iter % self.debug_img_interval == 0:
            out_dir = os.path.join(self.train_cfg['work_dir'], 'debug')
            os.makedirs(out_dir, exist_ok=True)
            if seg_debug.get('Source') is not None and seg_debug:
